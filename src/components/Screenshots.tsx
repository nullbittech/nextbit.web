          ...
          <div className="grid grid-cols-3 gap-2">
            {[1, 2, 3].map((i) => (
              <div key={i} className="bg-white/10 rounded p-2 h-12 flex items-center justify-center">
                <div className="text-white/50 text-xs">عنصر</div>
              </div>
            ))}
          </div>
        </div>
        ...
        <div className="grid grid-cols-4 gap-2">
          {[1, 2, 3, 4].map((i) => (
            <div key={i} className="bg-white/10 rounded p-3 text-center">
              <div className="text-white/50 text-xs mb-1">0</div>
    ...
    <section className="py-24 bg-gradient-to-b from-dark-900 to-dark-800 relative overflow-hidden">
      ...
      <div className="container mx-auto px-4 relative z-10">
        ...
        >
          ...
          <p className="text-xl text-gray-400 font-cairo">استعرض واجهات النظام الاحترافية</p>
        </motion.div>
                ...
                <div className="relative group">
                  ...
                  <div className="relative bg-white/5 backdrop-blur-xl border border-white/10 rounded-3xl p-8 hover:border-white/20 transition-all duration-500">
                    <div className="flex items-center justify-between mb-6">
                      <div>
                        <h3 className="text-2xl font-bold text-white font-cairo mb-1">{screenshot.title}</h3>
                        <div className="flex flex-wrap gap-2">
                          {screenshot.features.map((feature) => (
                            <span key={feature} className="text-xs px-2 py-1 bg-white/10 rounded-full text-gray-300 font-cairo">
                              {feature}
                            </span>
                          ))}
                        </div>
        ...
        >
          <p className="text-lg text-gray-400 font-cairo">
